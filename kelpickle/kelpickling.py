...
from kelpickle.common import NATIVE_TYPES, Jsonable, Json
...
from kelpickle.strategy_manager import get_strategy_by_name, get_strategy_by_type, register_strategy
...
class Pickler:
    ...
    def simplify(self, instance: Any) -> Jsonable:
    ...
    def simplify_with_strategy(self, instance: Any, strategy: Type[BaseStrategy]) -> Jsonable:
        simplified_instance = strategy.simplify(instance, self)
        if isinstance(simplified_instance, dict):
            simplified_instance[STRATEGY_KEY] = strategy.get_strategy_name()

        return simplified_instance

    @staticmethod
    def default_simplify(instance: Any, pickler: Pickler) -> Jsonable:
        instance_type = instance.__class__
...
def restore_by_dynamic_strategy(simplified_instance: Json, unpickler: Unpickler) -> Any:
    """
    ...
    strategy_name = simplified_instance.pop(STRATEGY_KEY)
    if strategy_name:
    ...
    return strategy.restore(simplified_instance, unpickler)


class Unpickler:
    simplified_type_to_restorer: dict[type, Callable[[Jsonable, Unpickler], Any]] = {
    ...
    def restore(self, simplified_instance: Jsonable) -> Any:
